    ...
    return (
        <section className="py-12 bg-gray-100">
            <div className="container mx-auto px-4">
                <h2 className="text-4xl font-bold text-center mb-8">{dict.latest_news}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {news.map((item) => {
                        ...
                        return (
                            <ItemNews lang={lang} key={item.id} title={title} dict={dict}  type={'news'}  item={item} slug={slug}/>
                        );
                    })}
                </div>
